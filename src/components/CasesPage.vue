<template>
  <div class="cases-page">
    <AppHeader description="Manage your dental cases and upload new files">
      <template #actions>
        <div class="user-info">
          <span class="welcome-text"
            >Welcome, <span class="username">{{ user?.username }}</span></span
          >
    ...
    <div
      v-if="showSegmentUploadModal"
      class="modal-overlay"
      @click="showSegmentUploadModal = false"
    >
      <div class="modal-content" @click.stop>
        <div class="modal-header">
          <h2>
            Upload Segment Files for {{ selectedCaseForSegments?.case_name }}
          </h2>
...
const deleteCase = async (caseId: any) => {
  const confirmed = await confirmDialogService.confirmDelete(
...
.upload-btn {
  ...
  cursor: pointer;
  transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1);
  box-shadow: 0 4px 16px rgba(81, 202, 205, 0.3),
...
.upload-btn:hover {
  ...
  box-shadow: 0 8px 24px rgba(81, 202, 205, 0.4),
...
.search-box:focus-within {
  ...
  box-shadow: 0 4px 20px rgba(81, 202, 205, 0.2),
...
.modal-content {
  ...
  max-width: 500px;
  width: 100%;
  box-shadow: 0 20px 60px rgba(0, 0, 0, 0.5), 0 8px 32px rgba(81, 202, 205, 0.1),
    inset 0 1px 0 rgba(81, 202, 205, 0.2), inset 0 -1px 0 rgba(0, 0, 0, 0.3);
  border: 1px solid rgba(81, 202, 205, 0.4);
