...
export class EnhancedBrushService {
   ...
   */
  async finishBrush(dentalModel: DentalModel): Promise<BrushOperationResult | null> {
    if (!this.isActive || !this.currentOperation || this.currentOperation.selectedVertices.size === 0) {
      this.cleanup()
      return null
    }
  ...
  private async addBrushPoint(
    ...
    let intersects = this.raycaster.intersectObject(dentalModel.originalMesh, false)
  ...
  private async selectVerticesAtPoint(
    ...
    const verticesToCheck = nearbyVertices.length > 0 ? nearbyVertices : 
      ...
      if (distance <= radius) {
        ...
        if (falloff >= (1 - this.currentSettings.strength)) {
          ...
          if (this.currentSettings.dentalAwareMode) {
            const isValid = await this.isDentalValid(vertexIndex, worldPosition, mesh)
            if (!isValid) {
              continue
            }
          }
  ...
  private async isDentalValid(
    ...
    const toBrush = new ThreeJS.Vector3().subVectors(brushCenter, vector).normalize()
  ...
  private async expandToConnectedRegion(
    ...
    while (queue.length > 0 && visited.size < 10000) { // Safety limit
   ...
   */
  private async processBrushOperation(dentalModel: DentalModel): Promise<BrushOperationResult> {
